                                ...
                                <tbody>
                                    @foreach ($roles as $rol)
                                        <tr>
                                            ...
                                            <td class="text-start">
                                                @if(!empty($rol->permissions))
                                                    ...
                                                    @if ($rol->name == 'super-admin')
                                                        <label class="badge badge-danger fs-6">Tiene todos los permisos</label>                                                    
                                                    @endif

                                                    @foreach($rol->permissions as $v)
                                                        <label class="badge badge-dark fs-6">{{ $v->name }},</label>
                                                    @endforeach
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                @can('role-show')
                                                <a href="{{ route('admin.roles.show', $rol->id) }}"><button class="btn btn-info mb-2 btn-sm"><i class="fa fa-fw fa-eye"></i>Ver</button></a>  
                                                @endcan
                                                @can('role-edit')
                                                <a href="{{ route('admin.roles.edit', $rol->id) }}"><button class="btn btn-warning mb-2 btn-sm"><i class="fa fa-fw fa-edit"></i>Editar</button></a>                                                   
                                                @endcan
                                                @can('role-delete')
